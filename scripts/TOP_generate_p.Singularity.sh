...
job1=$(sbatch --export=topdir=$topdir,mode=$mode,ref_txome=$ref_txome,KALLISTO_IDX=$KALLISTO_IDX,lib_size=$lib_size,scripts=$scripts --job-name="P_mat_replicates" --cpus-per-task=1 --time=4:00:00 --mem=40G --array=1-$reps --err=logs/err.%x.%j.log --output=logs/out.%x.%j.log --account=rrg-hsn tmp.sh)
echo $job1
...
#SBATCH --time=2:00:00
#SBATCH --mem=20G
job2=$(sbatch --dependency=afterany:$job1 --job-name="combine_replicate_counts" --output=logs/out.%x.%j.log --error=logs/err.%x.%j.log --time=2:00:00 --mem=20G --account=rrg-hsn --export=topdir=$topdir,reps=$reps,scripts=$scripts tmp2.sh) 
echo $job2
